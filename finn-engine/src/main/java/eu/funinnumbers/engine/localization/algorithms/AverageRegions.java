...
/**
 * Localization Algorithm using KNN and DatabaseLocalization for regions and points.
 */
public class AverageRegions implements Localization {
    /**
    ...
    /**
     * DatabaseLocalization Algorithm for points in a specific region.
     */
    private final DatabaseLocalization dbPoints = new DatabaseLocalization();

    /**
     * KNNLocalization Algorithm for points in a specific region.
     */
    private final DatabaseLocalization knnPoints = new DatabaseLocalization();
    ...
    public Point getCoordinates(final String macAddress) {
        ...
        final Point dbPoint = dbPoints.getCoordinates(currentNDPoint, regionPointsMap.get(averageRegionID));

        final Point knnPoint = knnPoints.getCoordinates(currentNDPoint, regionPointsMap.get(averageRegionID));

        final int averagePointID = (dbPoint.getId() + knnPoint.getId()) / 2;

        return new Point((float) averagePointID, dbPoint.getX(), averagePointID);
    }
    ...
    public void loadTrainedValues(final String regionsFile, final String pointsFile) {
        ...
        dbPoints.loadTrainedValues(pointsFile);
